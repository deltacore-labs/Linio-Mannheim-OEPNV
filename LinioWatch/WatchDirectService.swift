...
class WatchDirectService {
    ...
    private var hubIDsFetchTask: Task<[String], Never>? = nil
    private let hubIDsLock = NSLock()
    ...
    /// true wenn Credentials vorhanden, aber Token abgelaufen und Watch nicht selbständig authentifizieren kann
    var isTokenExpired: Bool {
        guard let creds = loadCredentials() else { return false }
        guard let expiry = creds.tokenExpiry else { return true }
        return Date().timeIntervalSince1970 >= expiry - 60
    }
    ...
    func getValidToken() async -> String? {
        ...
        guard let (token, expiry) = await Self.authenticate(creds: creds) else { return nil }
    ...
    private func getOrFetchHubIDs(token: String, graphQLURL: String) async -> [String] {
        hubIDsLock.lock()
        if let ids = cachedHubIDs { hubIDsLock.unlock(); return ids }
        if let task = hubIDsFetchTask { hubIDsLock.unlock(); return await task.value }
        let task = Task<[String], Never> {
            let ids = await withTaskGroup(of: String?.self) { group in
                for name in Self.hubStationNames {
                    group.addTask {
                        await Self.resolveStationID(name: name, token: token, graphQLURL: graphQLURL)
                    }
                }
                var result: [String] = []
    ...
    func fetchDepartures(stationID: String) async -> [WatchDeparture]? {
        ...
        let hubIDs = await getOrFetchHubIDs(token: token, graphQLURL: creds.graphQLURL)
        guard !hubIDs.isEmpty else { return nil }

        let time = Self.iso8601Formatter.string(from: Date())
    ...
    private static func authenticate(creds: Credentials) async -> (token: String, expiry: TimeInterval)? {
        // Watch hat kein clientSecret (Sicherheitsmaßnahme) und kann sich nicht selbständig
        // re-authentifizieren. Token muss vom iPhone via WatchConnectivity kommen.
        return nil
    }
    ...
    private static func resolveStationID(name: String, token: String, graphQLURL: String) async -> String? {
        ...
        guard let data     = try? await execute(query: query, token: token, graphQLURL: graphQLURL),
              let d        = parseResponseData(from: data),
    ...
    private static func parseResponseData(from data: Data) -> [String: Any]? {
